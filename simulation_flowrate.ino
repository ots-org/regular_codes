/******************************************************************************************************
 * 
 * Name         : simulation for  solenoid and flow sensor
 * 
 * Version      : 1.0
 * 
 * Author       : Raji
 ...
 * 
 * components   :flow sensor,solenoid,NRF,led's
 
 * Description: this has code of solenid nrf node with simulation in it and it has internal timer where 
                the motor switches off after some time and checks for amount of water in tank in the 
                absence of flow sensor
 *****************************************************************************************************/
...
/**********************make digital pin low*******************************/
digitalWrite(ERROR_LED,LOW);
digitalWrite(5,LOW);
digitalWrite(MOTOR,LOW);

 /*********************************Flowsensor begin *******************************************/
  flow_sensor_initialization(1);
/*********************attach interrupt**************************/  
 ...
 if((state==1)&&(this_node==5)&&(interupt_state==0))
   {
      ...
      radio.stopListening();
      strcpy(node_data,"");
      strcpy(node_data,"{\"R\":\"8\",\"ID\":\"OTSA031\"}");
...
/*******************************************INNTERUPT CALL***********************************************/   
   ...
   delay(1100);
   /******************************TOTAL TANK FLOWRATE IN LITRE THAT CHANGES************************************************/
      float eeprom_flowrate= EEPROM.read(20);
      Serial.println("eeprom_flowrate");
      Serial.println(eeprom_flowrate);
     /*********************get flowsensor rate*************************************/
     flow_rate=check_flow_sensor();
    /*****************************subract max.tank.liters and present litre flowed*************************************************/
    float stop_flow_rate  = eeprom_flowrate - flow_rate;// 300-20=280 LITERS IN THE TANK
     Serial.println("balance litre in tank");
     Serial.println(stop_flow_rate);
     /******************************store the total number of water flowed in eeprom***************************************************************/
       EEPROM.write(20,stop_flow_rate); //UPDATE THAT 280 IN EEPROM
      eeprom_flowrate=  EEPROM.read(20);
      /******************************flow sensor on/0ff**************************************************/
       int flowsensor_presence=  EEPROM.read(24);
   /*************************in the presense of flow sensor*************************************/
   if(1 == flowsensor_presence){
   /*****************************solenoid open*************************************/
   if(EEPROM.read(103)== 1){
   /*****************************but no waterflow send json there is no flow*************************************/
   if(flow_rate ==  0){
     ...
     /*****************************solenoid closed*************************************/
   if(EEPROM.read(103) != 1){
   /*****************************waterflow is there when solenoid closed*************************************/
   if(flow_rate !=  0){
       //send json to gateway
   }

   }
   /*************************condition for flow sensor*************************************/
   }
   /*************************in the absence of flow sensor*************************************/
   /*Total.lit of water*how much time flowed/total time to empty tank =remaining litres left in tank*/
    if(flowsensor_presence != 1){
      int total_water=EEPROM.read(26);
   // water_remaining= (total_water*)/water_remaining
    }
 } 
...
/******************************parse json action********************************************/
 ...
 for(itration2=itration2;itration2<=strlen(commands);itration2++)
   ...
   {  
    ...
    Serial.println(value_three);
   delay(99);
       Serial.println(add_constant);
     if((value_one=="900")&&(add_constant == 12))
      {
  ...
  /*******************************start sending to gateway*******************************************/      
       radio.stopListening();
       strcpy(node_data,"");
       strcat(node_data,"{\"R\":\"99\",\"ID\":\"OTSA031\"}");
       //strcat(node_data,new_number);
       //strcat(node_data,"\",\"T\":\"2\"}");
       delay(100);
 ...
 /***********************************delete the node address*****************************************/
  ...
  /*******************************start sending to gateway*******************************************/      
       radio.stopListening();
       strcpy(node_data,"");
       strcat(node_data,"{\"R\":\"2\",\"ID\":\"OTSA031\"}");
       //strcat(node_data,new_number);
       //strcat(node_data,"\",\"T\":\"2\"}");
       delay(100);
 ...
 /***********************************control solenoid*****************************************/
  ...
  /*******************************start sending to gateway*******************************************/      
       radio.stopListening();
        strcpy(node_data,"");
        strcat(node_data,"{\"R\":\"6\",\"A\":\"OTSA031\",\"S\":");
        strcat(node_data,string_motor.c_str());
        strcat(node_data,"}");
       delay(100);
...
/*******************************************
 *  FUNCTION: check_flow_sensor

    DISCRIPTION:reads the pulse pin and calculate the total 
                liters flowed  

    PARAMETERS: float

     RETURNS: total liters watered
 * 
 * 
 ******************************************/
float check_flow_sensor(){
      
     /*******************************************check flow sensor for every 1 second********************************************************/  
...
/**********************************************
     * FUNCTION: flow_sensor_initialization

       DISCRIPTION: initalize all the variables and pin
       if the sensor is present 

       PARAMETERS: int

      RETURNS: succesful initialization for flow sensor
 ************************************************/
int flow_sensor_initialization(int initializatrion_type){
